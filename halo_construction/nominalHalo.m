function [Y_xyz] = nominalHalo(mu,Lpoint,Ax,Az,m,plt)
% INPUT:
...
%
% OUTPUT:
%      Y_xyz = state vectrix for nominal halo over 1 closed orbit [nx6]
%%
clc; close all
...
DU = kmAU*ae; % distance unit
ne = sqrt((m1+m2)*G/((DU)*10^3)^3); n = 1; % SI and nondim mean motion
...
if plt==true
    ...
    JWST = readHorizons('horizons_results-7.txt',1); %wrt to Earth
    JWt = JWST{1}; % time (JD)
...
if plt==true
    ...
    plot3(Y_xyz(:,1), Y_xyz(:,2), Y_xyz(:,3), 'Color', [0.9 0.1 0.1], ...
        'LineWidth', 1.8)
    hold on;
